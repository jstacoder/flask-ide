...
from .forms import AddSSHServerForm, AddSSHServerAccountForm
...
class IndexView(BaseView):
    ...
    def post(self):
        result = {}
        result['success'],result['error'] = False,True
        form = self._form(request.form)
        fields = {}
        for field in form.data:
            fields[field] = form.data[field]
        server = Server().query.filter('ip_address'==fields['host']).first()
        if server is None:
            server = Server(name=fields['server_name'],ip_address=fields['host'])
            server.save()
        for account in fields['accounts']:
            tmp = Account(username=account['username'],password=account['password'],server_id=server.id,base_dir=account['base_dir'])
            tmp.save()
            result['success'],result['error'] = True,False
        return jsonify(form=fields,result=result)
